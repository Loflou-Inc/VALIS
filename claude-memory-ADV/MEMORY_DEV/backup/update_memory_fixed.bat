...
IF %FOUND_PYTHON% EQU 0 (
    ...
    IF %ERRORLEVEL% EQU 0 (
        python "G:\My Drive\Deftech\SmartSteps\claude-memory-ADV\MEMORY_DEV\simple_update_memory.py" %*
    ) ELSE (
        ECHO Python not found. Logging memory update to fallback file.
        ECHO %* >> "G:\My Drive\Deftech\SmartSteps\claude-memory-ADV\MEMORY_DEV\memory_store\memory_updates_fallback.txt"
    )
) ELSE (
    "%PYTHON_EXE%" "G:\My Drive\Deftech\SmartSteps\claude-memory-ADV\MEMORY_DEV\simple_update_memory.py" %*
)
