...
IF %FOUND_PYTHON% EQU 0 (
    ...
    IF %ERRORLEVEL% EQU 0 (
        python "G:\My Drive\Deftech\SmartSteps\claude-memory-ADV\MEMORY_DEV\smart_update_memory.py" %*
    ) ELSE (
        ECHO Python not found. Cannot update memory.
        GOTO END
    )
) ELSE (
    "%PYTHON_EXE%" "G:\My Drive\Deftech\SmartSteps\claude-memory-ADV\MEMORY_DEV\smart_update_memory.py" %*
)
